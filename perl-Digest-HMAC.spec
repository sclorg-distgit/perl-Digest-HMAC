%{?scl:%scl_package perl-Digest-HMAC}

Name:           %{?scl_prefix}perl-Digest-HMAC
Version:        1.03
Release:        25%{?dist}
...
# Run-time:
BuildRequires:  %{?scl_prefix}perl(Digest::MD5) >= 2
BuildRequires:  %{?scl_prefix}perl(Digest::SHA) >= 1
...
Requires:       %{?scl_prefix}perl(Digest::MD5) >= 2
Requires:       %{?scl_prefix}perl(Digest::SHA) >= 1

# Remove under-specified dependencies
%global __requires_exclude %{?__requires_exclude:%{__requires_exclude}|}^%{?scl_prefix}perl\\(Digest::(MD5|SHA)\\)$
...
%changelog
* Fri Feb 14 2020 Petr Pisar <ppisar@redhat.com> - 1.03-25
- Import to SCL
